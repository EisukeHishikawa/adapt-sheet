...
### レイアウトHTML生成 (PyMuPDF、backend内モジュール・ADR-025)

> ADR-025により、レイアウトHTML生成は独立コンテナ（旧pdf2htmlEXサービス）を廃止し、backend内の純Pythonモジュール（`backend/app/services/pdf_layout.py`、PyMuPDF）へ取り込んだ。PDFの1ページ目を、テキスト・罫線・背景を絶対座標のdivへ写した1枚のHTMLに変換する。専用のテスト・起動コマンドは無く、上記のbackend側コマンド（`docker compose exec backend pytest tests/test_pdf_layout.py -v` 等）で検証する。PyMuPDFはAGPL/商用ライセンスである点に留意（ADR-025のトレードオフ参照）。
