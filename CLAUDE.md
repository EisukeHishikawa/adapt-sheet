...
### pdf2htmlEX 品質評価用の検証環境 (Docker、ADR-023)

> Doclingの代替候補。実装へは未組み込みで、`tools/pdf2htmlex/`配下の検証専用コンテナのみ。公式イメージはx86_64のみのためApple Silicon上はエミュレーション実行（`platform: linux/amd64`）。

```bash
docker compose --profile pdf2htmlex run --rm pdf2htmlex        # input/配下の全PDFをoutput/へHTML化
docker compose --profile pdf2htmlex run --rm pdf2htmlex a.pdf   # ファイルを指定して変換
tools/pdf2htmlex/smoke-test.sh                                   # 検証環境自体の動作確認（フィクスチャPDFを変換）
tools/pdf2htmlex/compare-with-docling.sh /path/to/a.pdf          # Doclingの出力と並べて比較（要 docker compose up -d docling）
```
