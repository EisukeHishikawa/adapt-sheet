...
#### 定型サイズ自動入力
- A4たて/A4よこ/B5たて/B5よこ/A5たて/A5よこの6択を1つのSelectに統合する。トリガーは枠線・シェブロン等のフォーム部品的な装飾を持たず、選択中の用紙を実寸(mm)の縦横比で表した紙のイラストそのものがボタンになる。ドロップダウンを開くと同じ6択のイラストが縦一列に並ぶ。「たて」「よこ」の文字ラベルやmm表記は画面上に表示せず、方向はイラストの縦横比のみで、サイズ名(A4/B5/A5)はイラストの中に表記する（アクセシブルネームはaria-labelで別途保持）。選択時、対応する縦横の寸法を自動入力する。初期値はA4よこ（幅297mm・高さ210mm）。幅・高さの手動入力等どのプリセットとも一致しない寸法のときは、トリガーのイラストはサイズ名の表記が無い無印になるが、形（縦横比）は実際に入力された寸法をそのまま反映する。
