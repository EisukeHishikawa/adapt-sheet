...
def build_prompt(
    *,
    ...
    return (
        ...
        "明細（品目）の一覧は<table class=\"invoice-items\">で組み、セルのpadding（例: 4〜6px）と"
        "borderを整えて表として見やすくしてください。"
        "余白（margin/padding）と行間（line-height 1.3〜1.5程度）は、縮小した文字サイズに合わせて、"
        "詰まりすぎず間延びしすぎない、一般的な請求書として美しいバランスに調整してください。"
        "bodyや各要素のブラウザ既定marginはリセットし、要素間の余白は意図的に設計してください。\n"
        f"{size_line}"
