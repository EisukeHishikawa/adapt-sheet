...
# 内部契約（ADR-018/019）: POST /convert に file を送ると {"markdown": ...} が返る。
...
def test_convert_returns_markdown_on_success():
    class _FakeConverter:
        def convert_to_markdown(self, filename: str, content: bytes) -> str:
            return "# converted-markdown-marker"
    ...
    try:
        ...
        assert response.json() == {"markdown": "# converted-markdown-marker"}
    finally:
...
def test_convert_end_to_end_with_real_docling():
    ...
    assert response.status_code == 200
    assert "Docling verification sample text" in response.json()["markdown"]
