...
- **決定**:
  ...
  - **Gemini呼び出しの堅牢化**: `response_mime_type="application/json"`（コードフェンス・前置きの混入防止）と`max_output_tokens=16384`（長い帳票でJSONが途中で切れるのを防ぐ）を指定する。503（`ServerError`＝Gemini側の混雑）のみ指数バックオフで最大3回再試行し、429（クォータ超過）等のクライアントエラーは再試行しても結果が変わらないため即座に502で失敗させる。
  - **思考（thinking）の無効化と打ち切り検知**: `thinking_config=ThinkingConfig(thinking_budget=0)`で思考を無効化する。`gemini-2.5-flash`は思考モデルで、Gemini 2.5系では**思考トークンも`max_output_tokens`の予算を消費する**。既定の動的思考が予算の大半を食うとJSON本体が出力途中で打ち切られ「Unterminated string」で502になる事象が実際に発生した（`LOG_AI_PAYLOAD`で残した`ai_response`がJSONの途中で途切れていた）。帳票生成はレイアウトHTMLを保守しやすい構造へ作り替える構造化変換であり深い多段推論を要さないため、思考を止めて16384トークンを丸ごと出力へ充てる。あわせて、レスポンスの`candidates[0].finish_reason`が`MAX_TOKENS`のときは`parse_ai_response`（不正JSON扱い）の前に、原因（上限到達）と対処（入力短縮・`GEMINI_MODEL`変更）が分かる`AIGenerationError`を送出する（`_raise_if_truncated`。`finish_reason`は`.name`で文字列比較しSDKの列挙体表現差に依存しない）。単に`max_output_tokens`を増やすだけでは動的思考も比例して膨らみうるため確実な解にならない。将来、思考を活かしたい場合は`thinking_budget`に有限値を与えつつ`max_output_tokens`を思考分だけ上積みする方式へ移行できる（`thinking_budget=0`はその特殊ケース）。
...
## ADR-020: コードコメントは「なぜ」だけを書き、経緯はADRへ集約する
...
## ADR-021: Git Worktree・ブランチは最小構成（本体＋docs-space・`worktree-*`ブランチを残さない）に保つ
...
## ADR-022: Geminiの入出力全文ログを`LOG_AI_PAYLOAD`によるオプトインで出す
