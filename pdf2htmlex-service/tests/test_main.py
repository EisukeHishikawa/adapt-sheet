...
def test_convert_returns_html_on_success():
    class _FakeConverter:
        def convert_to_html(self, filename: str, content: bytes) -> str:
            return "<html><body>pdf2htmlex-converted-marker</body></html>"
    ...
    try:
        ...
        assert response.json() == {"html": "<html><body>pdf2htmlex-converted-marker</body></html>"}
    finally:
...
def test_convert_end_to_end_with_real_pdf2htmlex():
    # DIをフェイクに差し替えず、実際のpdf2htmlEXバイナリを通す結合確認。
    response = client.post(
        "/convert",
        files={"file": ("sample.pdf", SAMPLE_PDF.read_bytes(), "application/pdf")},
    )
    assert response.status_code == 200
    assert "<html" in response.json()["html"].lower()
