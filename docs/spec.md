...
- **登録ユーザー**: Supabase Authでログインし、`POST /api/render`成功時に生成履歴が自動的にSupabase（PostgreSQL）へ保存される。保存済み履歴は`HistorySlider`（最大10件、セッション切れ後もログイン確定時に再取得）と`HistoryArchive`（それより古い過去データ。キーワード・種類での絞り込みと1ページ20件ずつの追加取得）で閲覧・削除できる。
...
#### 過去データの検索・削除（HistoryArchive）
- 10件枠の外にある保存済み履歴は「過去データを見る」から一覧する。履歴が増えても全件は読まず、1回20件ずつ取得して「さらに読み込む」で続きを足す。
- キーワード欄は入力が止まった時点で自動的に検索し直す（サーバー側でHTMLと業務データを部分一致）。種類（すべて/描画結果/編集中）も併せて絞り込める。
- 各行のゴミ箱アイコンから履歴を1件削除する。誤操作で消えないよう、押した直後は「削除する / 取消」の確認に切り替わる。
...
ログイン中のユーザーが`POST /api/render`で成功させた生成履歴を、新しい順に1ページ分返す。`POST /api/render`成功時にサーバー側で自動保存されるため、専用の保存操作（保存ボタン等）はない。

**リクエスト**

`Authorization: Bearer <Supabaseアクセストークン>`ヘッダーが必須。未ログイン（ヘッダー無し・無効なトークン）の場合は`403 FREE_ACCESS_FORBIDDEN`を返す（4章参照）。

履歴が増えても全件を読まないよう、絞り込みとページングはSQL側で行う。

| クエリパラメータ | 型 | 既定 | 説明 |
|---|---|---|---|
| `q` | string | なし | HTMLと業務データ（JSON）に対する部分一致・大文字小文字を区別しない検索。200文字まで |
| `kind` | `render` \| `edit` | なし（両方） | 描画結果／編集中スナップショットの絞り込み |
| `limit` | int | 20 | 1回に返す件数。1〜50 |
| `offset` | int | 0 | 続きを読むための開始位置 |
...
### 3.5 DELETE /api/history/{id}（登録ユーザー限定）

自分の履歴1件を削除する。成功時は`204 No Content`（ボディなし）。

**エラー**

| HTTPステータス | 説明 |
|---|---|
| `403 Forbidden` | 未ログイン（`FREE_ACCESS_FORBIDDEN`） |
| `404 Not Found` | 他ユーザーの履歴・存在しないID・UUIDとして解釈できないID |

### 3.6 `POST /api/warmup`（ホットスタンバイ）
...
| `502 Bad Gateway` | `AI_GENERATION_ERROR` | AI生成エラー | Gemini/Claude/OpenAI API呼び出し失敗（Geminiの503以外の5xx・404を含む）、タイムアウト、不正なレスポンス形式 |
