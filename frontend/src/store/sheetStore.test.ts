...
const initialSheetState = {
  ...
  widthMm: null,
  heightMm: null,
  // ADR-016で追加したエンジン選択。setStateは浅いマージのため、リセットに含めないとテスト間で
  // 前回選択したengineが漏れる。
...
describe('sheetStore（ステータスコード準拠のエラー/成功メッセージ）', () => {
  ...
  it('バックエンドの構造化エラーボディのmessageを、ステータス別の既定文言より優先して表示する（ADR-013）', async () => {
...
// 一致する形で送信されること、およびADR-015に基づきcssフィールドが送信されないことを検証する。
// jsonContent（業務データJSON）はGeminiへの入力として不要になったため送信されない
// （backend/app/main.pyのjson_fieldパラメータ廃止と対）。
describe('sheetStore（JSON/プロンプト入力欄の送信・ADR-015）', () => {
  beforeEach(() => {
    useSheetStore.setState(initialSheetState)
  })
  ...
  it('promptContentの値がfetchRenderでリクエストのpromptフィールドとして送信され、jsonContentは送信されない', async () => {
    ...
    // ADR-015: cssは独立したリクエストフィールドを持たないため、送信されないことを固定する。
  ...
  it('htmlContentはfetchRenderで送信されない（ADR-016：生成AIへPDFを直接送るため不要）', async () => {
    let capturedFormData: FormData | undefined
    server.use(
      http.post('/api/render', async ({ request }) => {
        capturedFormData = await request.formData()
...
// ADR-016のTDD要件: EngineSelectで選択したengineがfetchRenderのリクエストへ反映されること、
// 既定値がgemini_free（無料枠）であることを検証する。
describe('sheetStore（モデル選択・ADR-016）', () => {
  beforeEach(() => {
    useSheetStore.setState(initialSheetState)
  })
