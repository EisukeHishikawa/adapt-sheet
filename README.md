...
### Doclingを直接使ったファイル出力（アプリを介さない動作確認）

`backend`のAPI（`/api/render`等）を経由せず、Docling自体の変換結果をファイルとして手元に取り出したい場合の手順。`docling`コンテナはホストへポートを公開していないため、`docker compose exec`でコンテナ内から直接`DocumentConverter`を呼び出す。

```bash
# 1. 変換したいPDFをホストからdoclingコンテナへコピー
docker compose cp "/path/to/input.pdf" docling:/tmp/input.pdf

# 2. コンテナ内でDoclingを実行し、HTMLへ変換して/tmp配下に書き出す
docker compose exec docling python -c "
from docling.document_converter import DocumentConverter
converter = DocumentConverter()
result = converter.convert('/tmp/input.pdf')
...
# 3. 出力結果をホスト側へ取り出す
docker compose cp docling:/tmp/output.html "/path/to/output.html"
```
