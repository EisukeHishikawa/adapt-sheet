...
def test_build_prompt_instructs_treating_pdf_as_visual_source_when_has_pdf():
    # PDFが添付されている場合、それを見た目の正として扱う指示を含む。
...
def test_mock_client_ignores_pdf_argument():
    # どのエンジンが選ばれてもUSE_MOCK_AI=trueの間はMockAIClientが使われるため、
...
# MockAIClientはbuild_promptが埋め込んだ帳票サイズから用紙の向きを判定し、
...
def test_get_ai_client_defaults_to_mock(monkeypatch):
    # 環境変数未設定時はテスト・ローカル開発を安全にするためモックを既定にする。
...
# engine引数（EngineSelectの選択値）が実クライアントの選択を決める契約を検証する。
...
# Gemini APIは高負荷時に503 UNAVAILABLE（"experiencing high demand"）を返すことがある。
...
def test_gemini_client_sends_pdf_as_part_when_provided():
    # PDFがある場合、PyMuPDF/Docling経由の事前変換を挟まずマルチモーダル入力として渡す。
    models = _StubGeminiModels(failures=0, response_text=_VALID_RESPONSE)
    client = GeminiAIClient(api_key="dummy", client=_StubGeminiClient(models))
...
def test_gemini_client_does_not_log_payload_by_default(monkeypatch, caplog):
    # 既定では帳票の業務データを含む全文をログに残さない（機微情報の非出力）。
...
# Claude APIクライアント。PDFはbase64のdocument content blockとして直接添付する。
...
# OpenAI APIクライアント。Responses APIでPDFをbase64のinput_fileとして直接添付する。
