"""PyMuPDF（fitz）によるレイアウトHTML生成（ADR-014）。

PDFの1ページ目を、テキスト・罫線・背景色を絶対座標のdivへ写した1枚のHTMLに変換する。
出力の唯一の宛先はGeminiのプロンプトであり、Geminiがこれを「見た目の正」として読み、
保守しやすいHTML/CSSへ作り替える（テキストの正はDocling由来のMarkdown。ADR-014の役割分担を踏襲）。
"""
...
# 一般的な請求書・帳票として過大にならないフォントサイズ上限（px）。役割（エリア）別に分ける（ADR-014）。
...
class PyMuPDFLayoutConverter:
    """PDFの1ページ目をレイアウト保持HTMLへ変換するbackend内実装（ADR-014）。"""
