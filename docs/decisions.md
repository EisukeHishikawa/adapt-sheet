...
- **既知の課題（CI）**: `docling`ジョブの実PDF変換テストは初回実行時にHugging Faceからレイアウトモデルをダウンロードするため、外部到達性に依存しCIが不安定になることがある。Branch Protectionの必須チェック化（DEVELOPMENT.md ステップ26）により影響が顕在化したため、`docling-service/Dockerfile`へモデルを事前焼き込みする対応を別PRで行う（関心事の分離）。当面失敗した場合は該当ジョブの再実行で回避する。
