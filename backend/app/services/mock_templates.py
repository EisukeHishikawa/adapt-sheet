"""MockAIClientが返す帳票モック（ADR-020）。縦=納品書 / 横=請求書の2種類のみ。

CSSの寸法をpxではなくvw（用紙の実寸幅に対する相対値）にしているのは、A4/B5/A5のどれでも
同じHTML/CSSのまま文字が用紙に収まるようにするため（PreviewPanelがiframeを用紙の実寸pxで
組版するため、vwは用紙幅に連動する）。

固定テキスト（発行者・ラベル・敬称）はHTMLへ直書きし、業務データのみ{{key}}にする
（CLAUDE.md「固定情報と業務データの分離」）。フロントのテンプレート置換はトップレベルキーの
単純な文字列置換のみのため、ネスト・配列は使わず明細行も行番号を含むキー名でフラットに持つ。
"""
