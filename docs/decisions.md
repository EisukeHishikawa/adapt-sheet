...
- **ステータス**: 一部Superseded（「見た目＝レイアウトHTML」「テキスト＝Docling Markdown」の役割分担と並列実行の枠組みは有効。ただしレイアウトHTMLの生成手段はpdf2htmlEX専用コンテナからPyMuPDF（backend内）へADR-025で置き換えた）
...
## ADR-024: 罫線・枠線をベクター（インラインSVG）としてGeminiへ渡す

- **ステータス**: 一部Superseded（pdf2htmlEX/SVGによる罫線出力の部分はADR-025で置き換え。ただし「テンプレート変数の欠けを502にせず補完する」判断は現行仕様として有効）
- **コンテキスト**: ADR-023では、pdf2htmlEXのトークン量を抑えるため`--process-nontext 0`（罫線・図形などの非テキスト要素を出力しない）とし、「罫線・表構造はDocling由来のMarkdownから読み取らせる」前提を置いた。しかし実際の帳票PDF（請求書テンプレート）で検証したところ、この前提が誤っていた。pdf2htmlEXの出力は**文字だけ**で、表の枠線・区切り線・網掛けが完全に欠落していた。そしてDoclingの`export_to_markdown`は罫線を表現できない（Markdownには罫線という概念が無く、値の入っていない空テンプレートでは表構造にすら復元されず、ラベルの平文列挙に落ちる）。結果として、**罫線の情報がどちらの入力にも含まれず**、Geminiは表の枠を再現できなかった。
- **決定**: pdf2htmlEXで罫線・図形を**ベクター（SVG）として出力し、HTMLへインライン展開して**Geminiへ渡す。
  - `--process-nontext 1`（非テキスト要素を処理）＋`--bg-format svg`（背景をラスタライズせずSVGで生成）＋`--svg-embed-bitmap 0`（SVG内へビットマップを埋め込ませない）を指定する。罫線はSVGの`<path>`（座標付きの線・矩形）になる。
  - pdf2htmlEXはSVGを外部ファイル（`<img class="bf" src="bgN.svg">`）として出力するが、backendが受け取るのはHTML文字列1枚だけでGeminiも外部ファイルを読めない。そのため変換後に**SVGをインラインの`<svg>`へ展開**し、元の`<img>`が持つ位置指定クラス（`.bf`）を`<svg>`へ引き継いでページとの位置ずれを防ぐ（`_inline_svg_backgrounds`）。
  - build_prompt側で「`<svg>`のpathは罫線・枠線・網掛けを表す。読み取って`<table>`のborderやCSSのborder/background-colorとして再現し、`<svg>`自体は出力に含めない」とGeminiへ役割を明示する。
- **理由**:
  - 罫線は帳票の見た目の中核（表の構造そのもの）であり、これが欠けると再現の質が大きく落ちる。見た目の正はpdf2htmlEX側という役割分担（ADR-023）を、罫線についても貫徹する。
  - ラスタライズ（PNG）ではなくベクター（SVG）を選ぶのは、base64ビットマップがLLMには読めずトークンだけ消費するため（ADR-023でbase64を排除した判断と同じ）。SVGの`<path>`は座標付きテキストであり、Geminiが線の位置・長さ・囲みを解釈できる。
- **トレードオフ**:
  - 入力トークンが増える（実測: 請求書テンプレートでHTML 15,678字 → 44,618字。罫線122本ぶんの`<path>`が加わる）。ADR-023の503対策（base64排除・再試行）は維持しており、ベクターのpath自体は軽量なため、この増加は許容範囲と判断した。トークンが問題になる場合は`PDF2HTMLEX_PROCESS_NONTEXT=0`で従来の文字のみ出力へ戻せるよう環境変数化してある。
  - SVG内にロゴ等のビットマップ（data URI）が残る場合があるため、`<image>`要素は展開時に除去する（罫線の情報は`<path>`だけで足りる）。
  - （ADR-025補足）SVGインライン展開によるトークン増加と、pdf2htmlEXの保守しづらい出力構造の両方が、ADR-025でPyMuPDFへ移行する動機になった。PyMuPDFでは罫線・塗りを座標付きの`border-element`/`bg-element`のdivとして直接出力するため、SVG経由の迂回が不要になった。
- **テンプレート変数の欠けを502にせず補完する**: 実Geminiを有効化して罫線入りの請求書テンプレートを変換したところ、生成HTMLに`{{item_1_description}}`等のプレースホルダがある一方、その明細行が空欄だったためGeminiが対応するjsonキーを落とし、`validate_render_result`が502で失敗した。プロンプト側は「htmlの`{{key}}`とjsonキーを過不足なく一対一で対応させる（空欄セルでも空文字列でキーを含める）」を絶対厳守ルールへ格上げして再発を抑える。加えて、LLMの出力は確率的で完全な保証はできないため、`validate_render_result`は欠けたキーを**空文字列で補完**してレンダリングを成立させる方針へ変更した。1件のキー漏れで帳票全体を502にするより、空欄セルとして描画するほうがUXが良く、これはモデルが値を出さなかった意図にも忠実である。htmlに現れないjsonの余剰キーは、テンプレート適用時に使われないだけで無害なため許容する（従来は余剰キーも許容していた挙動を踏襲）。html/css自体が空、jsonがオブジェクトでない等の重大な契約違反は従来どおり502で失敗させる。この判断はADR-025移行後も有効。

---

## ADR-025: レイアウトHTML生成をpdf2htmlEX専用コンテナからPyMuPDF（backend内モジュール）へ置き換える

- **ステータス**: Accepted
- **コンテキスト**: ADR-023でレイアウトHTML生成にpdf2htmlEXを採用したが、運用してみて次の問題が明らかになった。
  - **出力が保守しづらい**: pdf2htmlEXの出力は絶対座標の`<div>`と機械生成された巨大なCSSクラス群で、Geminiが「見た目の正」として読むには冗長で、構造を読み解く負担が大きい。
  - **罫線の扱いが厄介**: 罫線は既定では背景画像（ラスタライズ）になりLLMに読めない。ADR-024でSVGインライン展開に切り替えて回避したが、トークンが増え（実測15,678→44,618字）、SVG内ビットマップの除去など後処理も増えた。
  - **専用コンテナの重さ**: pdf2htmlEXはPoppler/FontForgeに依存するC++バイナリで、公式イメージがx86_64のみ・最終更新2020年。Apple Silicon上はエミュレーション実行になり、コンテナのビルド・起動コストも大きい。
- **決定**: レイアウトHTML生成を、純Pythonの[PyMuPDF](https://pymupdf.readthedocs.io/)（`fitz`）を使ったbackend内モジュール（`backend/app/services/pdf_layout.py`）へ置き換え、pdf2htmlEX専用コンテナ（`pdf2htmlex-service`）を廃止する。ADR-023の役割分担（レイアウト＝HTML、テキスト＝Docling Markdown）と並列実行の枠組みはそのまま維持する。
  - `page.get_text("dict")`でテキストを座標・フォントサイズ・色・フォント名付きで取得し、`class="text-element"`の絶対配置divへ写す。フォント名に`bold`/`black`/`heavy`/`gothic`を含むものは`font-weight:bold`にする。
  - `page.get_drawings()`で罫線（stroke, type=`s`）を`class="border-element"`、背景の塗り（fill, type=`f`）を`class="bg-element"`のdivへ写す。座標・色はPyMuPDFが直接返すため、SVGやラスタライズを経由しない。
  - ページサイズ（pt）をそのまま`.page-container`のpxに使い、絶対配置の基準にする。埋め込みフォントが無い環境でも字形が崩れないよう、Webフォント（Noto Sans JP）を`font-family`最優先で指定する。
  - `PDFLayoutConverter`プロトコルと`get_layout_converter`ファクトリ名はADR-023から引き継ぎ、backendのDI（`Depends`）・テスト（`dependency_overrides`）はそのまま使える。HTTP委譲（`RemotePdf2htmlEXConverter`）は不要になり削除した。
  - build_promptは、`<svg>`前提（ADR-024）から`text-element`/`border-element`/`bg-element`のdivを罫線・背景として読み取らせる指示へ更新した。
- **理由**:
  - 純Pythonでbackendへ直接importでき、専用コンテナ・x86_64エミュレーション・HTTPホップ・専用イメージのビルドがすべて不要になる。コンテナ数はfrontend/backend/doclingの3つに戻る。
  - 罫線・塗りを座標付きの図形として直接取得できるため、ADR-024で導入したSVGインライン展開・ビットマップ除去の後処理が丸ごと不要になる。出力もpdf2htmlEXより素直な絶対配置divで、Geminiが読みやすい。
- **トレードオフ**:
  - **ライセンス**: PyMuPDFはAGPL v3（または商用ライセンス）。本番でSaaSとして提供する場合、AGPLの適用範囲を検討する必要がある。ユーザーの明示選択のうえで採用した。将来ライセンスが問題になる場合は、MITの`pdfplumber`等への再置換を検討する（本ADRの実装は`pdf_layout.py`に閉じており差し替えやすい）。
  - **フォント太字の推定**: PDFは太字を別フォントとして持つことが多く、CSSのfont-weightへ機械的には写らない。フォント名からの推定（`bold`/`gothic`等）に留め、完全一致は求めない。見た目の最終調整はGeminiが担う。
  - **描画種別の網羅**: `get_drawings()`の`type`は`f`（塗り）/`s`（線）/`fs`（両方）等がある。実装では`f`と`s`を扱い、極小の塗り（1px以下、字の影・アンチエイリアス由来）は除外する。細部の欠けはGeminiがMarkdownと突き合わせて補う前提。

---

## ADR-026: フォントサイズを役割別の上限で頭打ちにし、Geminiへも過大化しないよう指示する

- **ステータス**: Accepted
- **コンテキスト**: PyMuPDF版のレイアウトHTML（ADR-025）は、PDF上のフォントサイズ（pt）をそのままpxに写す。多くの帳票では適切だが、大きめの字で作られたPDFではそのまま大きなfont-sizeになり、Geminiの生成HTMLも過大なフォントになりやすい（Geminiは入力HTMLを見た目の参照にするため）。「一般的な請求書・帳票として詰まって美しいサイズ」に収めたい。
- **決定**: 2段構えでフォントサイズを抑える。
  - **レイアウトHTML側（`pdf_layout.py`）**: フォントサイズを役割別の上限で頭打ちにする（`_capped_font_size`）。元サイズから役割を推定し、タイトル22px・見出し14px・明細/本文/その他11pxを上限とする。上限以下の小さい字は縮めない（`min`）。明細とその他はフォントサイズだけでは区別できないため、同じ本文サイズ帯として1つの上限にまとめる。
  - **Geminiプロンプト側（`build_prompt`）**: 「フォントサイズは元より大きくしない」「タイトル22px以下・見出し14px以下・明細/本文/その他11px以下」「本文・明細は10〜11px程度で余白と行間で見やすさを確保」と明示する。実際に見た目を作るのはGeminiのため、こちらが最終的な効き目を持つ。
- **理由**:
  - 入力（レイアウトHTML）と指示（プロンプト）の両方でサイズ上限を揃えると、Geminiへ一貫した基準を与えられる。レイアウトHTML側の上限は過大フォントのPDFに対する安全弁、プロンプト側は最終出力の主たる制御。
  - 上限は用途別に分ける（帳票名は大きめ、本文は小さめ）ことで、階層感を保ちつつ全体を詰まった帳票サイズへ収める。
- **トレードオフ**:
  - 役割の推定は元のフォントサイズ（pt）の閾値で行うため厳密ではない（例: 見出しと本文が同じ大きさのPDFでは区別できない）。上限は緩めに設定し、階層が崩れない範囲で頭打ちにするに留める。閾値・上限は`pdf_layout.py`の定数で調整できる。
  - 元から上限以下のPDF（実サンプルの請求書テンプレートは本文8px・タイトル18.5px）ではレイアウトHTMLは変化せず、改善はもっぱらプロンプト側の指示で得られる。
- **見出しタグの既定サイズが主な過大要因**: 実Geminiは意味的な構造として`<h1>〜<h3>`を出力するが、これらはブラウザ既定のfont-sizeが大きい（h1は約32px）。CSSでfont-sizeを明示的に上書きしないと、px上限を指示しても見出しが大きく表示される。プロンプトで「見出しタグは既定サイズのまま使わず必ず上限内へ上書きする」ことと、明細の`<table class="invoice-items">`化・縮小に合わせた余白（margin/padding・line-height）の調整を明示的に指示する。
...
- **決定**: `backend/app/services/ai_client.py`の`GeminiAIClient.generate`に、環境変数`LOG_AI_PAYLOAD`でオプトインする全文ログを追加する。
  ...
  - **対象はGemini経路のみ**: `MockAIClient`の出力は固定テンプレート（ADR-020）でログの価値が無く、`LlamaAIClient`はADR-013で既定経路から外れているため、本ADRの対象に含めない。
...
- **関連**: ADR-016（構造化ログ基盤・機微情報の非出力）、ADR-017（生の例外メッセージはレスポンスに載せずログにのみ残す）、ADR-023（`parse_ai_response`が扱うGeminiレスポンスの契約）。
