...
const initialSheetState = {
  ...
  widthMm: null,
  heightMm: null,
  // ADR-023で追加したエンジン選択。setStateは浅いマージのため、リセットに含めないとテスト間で
  // 前回選択したengineが漏れる。
...
describe('sheetStore（JSON/プロンプト入力欄の送信・ADR-019）', () => {
  ...
  it('htmlContentはfetchRenderで送信されない（ADR-023：生成AIへPDFを直接送るため不要）', async () => {
    let capturedFormData: FormData | undefined
    server.use(
      http.post('/api/render', async ({ request }) => {
        capturedFormData = await request.formData()
...
// ADR-023のTDD要件: EngineSelectで選択したengineがfetchRenderのリクエストへ反映されること、
// 既定値がgemini_free（無料枠）であることを検証する。
describe('sheetStore（モデル選択・ADR-023）', () => {
  beforeEach(() => {
    useSheetStore.setState(initialSheetState)
  })
