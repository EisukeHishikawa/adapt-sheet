...
- [ ] TerraformによるCloudFront + S3、AWS Lambda（メモリは余裕を持たせた4GB〜8GB推奨） + API Gateway、AWS WAF、**ECR Public（`public.ecr.aws`。ECR Privateの無料枠500MBを避けるため。ADR-017）** によるインフラの定義・構築。APIキーはSecureStringのSSM Parameter Storeで管理し、Lambdaの実行ロールに`ssm:GetParameters`/`kms:Decrypt`を付与する
- [ ] 🧪 **ステージングテスト:** デプロイされたクラウド環境のエンドポイントに対して、ローカルからAPIテストを実行して疎通を確認

#### ⬛ ステップ 26: GitHub ActionsによるCI/CDの構築 【自動テスト化】
...
- [ ] フロントにSupabase Auth SDK組み込み。バックにJWT認証ミドルウェアを実装
- [ ] ⚙️ **モデル選択機能のゲート解除（ADR-015）:** `app/main.py`の`GATED_ENGINES`判定を、未ログイン時のみ403を返すよう条件を差し替える（Gemini標準/Claude/OpenAIクライアント自体はステップ23で実装済み）
