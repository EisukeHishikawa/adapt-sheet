...
## ADR-023: DoclingのHTML変換精度不足を受け、pdf2htmlEXを代替候補として先に品質評価する

- **ステータス**: Proposed（品質評価の結果を待って Accepted / Rejected を確定する）
- **コンテキスト**: ADR-003で採用したDoclingは、PDFから**意味構造**（見出し・段落・表）を抽出してHTML化するツールであり、`export_to_html()`の出力は座標・罫線・フォント・余白といった**見た目**を保持しない。adapt-sheetは「既存帳票の見た目を再現したHTML/CSSを作る」ことが中核体験であり、ADR-020ではDocling由来HTMLの見た目再現をGeminiのプロンプト側で補う方針を取ったが、そもそもの入力にレイアウト情報が無いため精度が頭打ちになっている。
- **決定**: PDFを絶対座標のdiv＋埋め込みフォント/画像として忠実にHTML化する [pdf2htmlEX](https://github.com/pdf2htmlEX/pdf2htmlEX) を代替候補とする。ただし**先に実装へ組み込まず**、`tools/pdf2htmlex/`（検証専用のDockerイメージ＋変換スクリプト、`docker-compose.yml`の`pdf2htmlex`プロファイル）で実PDFの出力品質を目視評価する段階を挟む。評価結果が良ければ、次のステップで`docling-service`の置き換え（またはコンバータ差し替え）をTDDで実装する。
- **理由**: Doclingは重量級のML依存（torch/opencv）を持ち、ADR-018でコンテナ分離までして組み込んだ経緯がある。置き換えは影響範囲が大きいため、「見た目の再現度が実際に十分か」を実PDFで先に確認してから着手する方が手戻りが小さい。pdf2htmlEXはPopplerベースでMLモデルを持たず、モデルのダウンロード・焼き込み（ADR-004）も不要になるため、コールドスタート面でも有利になる可能性がある。
- **トレードオフ**:
  - pdf2htmlEXは見た目を再現する一方、Doclingのような**意味構造の抽出は行わない**。「どこが明細行か」といった構造化は、生成AI側（Gemini）が座標付きHTMLから読み取る前提になる。見た目の再現度と構造化のしやすさはトレードオフであり、評価時は両面を見る。
  - 公式Dockerイメージはx86_64ビルドのみのため、Apple Silicon上の開発機ではエミュレーション実行になる（`platform: linux/amd64`）。本番のAWS Lambda（x86_64）ではネイティブ実行になるため、この制約は開発環境に閉じる。
  - pdf2htmlEXの最終リリースは2020年（0.18.8.rc2）で更新が停滞している。採用する場合は、この点を踏まえた上で「PDFレンダリングはPopplerに依存しており枯れている」ことを許容する判断になる。
- **評価環境の既定値**: 帳票は1ページ完結が前提（ADR-021）のため既定は1ページ目のみ変換。CSS/フォント/画像/JSは全て埋め込み、単一HTMLとして出力する（ブラウザでそのまま開け、AIへも1ファイルで渡せるため）。
