...
## ADR-027: WAFを使わず、API Gatewayのステージ単位スロットリングでレート制限する

- **ステータス**: Accepted
- **コンテキスト**: 当初はAWS WAF（AWSマネージドルール＋IPアドレス単位のレートベースルール）をAPI Gatewayステージの前段に置く設計だった（ADR-004時点の構成）。しかしWAF Web ACLはリクエスト数に関わらず固定費（Web ACL本体$5/月＋ルールごとに$1/月）が発生し、ソロ開発・低トラフィックの現段階ではインフラ費用の大半をWAFの固定費が占めてしまう。
- **決定**: WAFを廃止し、API Gatewayの標準機能であるステージ単位のスロットリング（`aws_api_gateway_method_settings`、`method_path = "*/*"`でステージ全メソッドへ適用）へ置き換える。`infra/modules/waf/`は削除し、`infra/modules/api_gateway/`に`throttle_rate_limit`（定常req/秒、既定50）・`throttle_burst_limit`（バースト、既定100）を追加する。
- **理由**: API Gatewayのスロットリングは追加コストなし（API Gateway自体の従量課金のみ）で、レート制限という当面の目的（過度なAPIコール・無料枠AI呼び出しの濫用防止）は満たせる。
- **トレードオフ**: WAFのレートベースルールは**IPアドレスごとに独立してカウント**するのに対し、API Gatewayのスロットリングは**ステージ全体の合算**でカウントする。そのため1クライアントが上限まで連打すると、他の正当な利用者のリクエストも一時的に捌けなくなりうる（`docs/spec.md`の`429 RATE_LIMITED`の説明を「未認証エリアのIP単位」から「ステージ全体の合算」へ変更した）。また、AWSマネージドルールセットによるSQLi/XSS等の既知攻撃パターン検知は失われる（FastAPI/Pydanticの入力バリデーションが一次防御として残る）。将来的にトラフィックが増え、IP単位の防御が必要になった場合はWAFの再導入を検討する。
- **関連**: ADR-004（Terraform IaC一本化）。

---

## ADR-028: フロント表示時のホットスタンバイ（docling/pdf2htmlex Lambdaの空打ち＋Supabaseキープアライブ）

- **ステータス**: Accepted
- **コンテキスト**: `docling`/`pdf2htmlex`はLambda化（ADR-026）により、実行環境が無い状態からの初回リクエストでコンテナイメージの展開・依存のロード（doclingはMLモデルのダウンロードを含む）が走り、体感で数十秒待たされる。この待ち時間は「PDFを投入して描画を押した瞬間」に発生するため、利用者からは描画そのものが遅いように見える。また、Supabaseの無料プロジェクトは一定期間アクセスが無いと一時停止され、復帰まで手作業が必要になる。
- **決定**: 画面（フロント）を開いた時点で、実際の描画とは無関係な軽量リクエストを依存先へ投げてウォームアップする。
  - `docling-service`/`pdf2htmlex-service`に**`GET /health`**（PDF変換のDIに一切触れず`{"status": "ok"}`を返すだけ）を追加する。
  - backendに**`POST /api/warmup`**（認証不要）を追加し、両サービスの`/health`とSupabaseのDBへの`SELECT 1`を並行実行する。両Lambdaはbackendの実行ロールしか呼べないIAM認証必須のFunction URL（ADR-026）のため、フロントからは直接叩けず、backendが`RemoteHtmlExtractor.warmup()`でSigV4署名して代理ピングする。
  - フロントは`App`のマウント時に`runWarmup()`（`src/lib/warmup.ts`）を投げっぱなしにする。内訳は`POST /api/warmup`と、SupabaseへのSDK経由の最小クエリ（`render_history`を`limit(1)`でSELECT）。
  - **ウォームアップの失敗は一切UIへ出さず、例外にもしない**。`/api/warmup`はどの対象が失敗しても200を返し、対象ごとに`"ok"`/`"unavailable"`を報告するだけに留める（サーバー側は`WARNING`ログを残す）。
- **理由**: 利用者がPDFを選び終えるまでの数十秒を、コールドスタートの解消に充てられる。ウォームアップ経路を専用の軽量エンドポイントにすることで、変換処理（重い・課金される生成AI呼び出しを含みうる）を空打ちせずに済む。Supabaseのキープアライブも同じ導線に相乗りさせ、「画面が開かれている＝プロジェクトが使われている」状態を保つ。
- **トレードオフ**: 画面表示のたびにLambda 2本の呼び出しとDBクエリが1回ずつ増える（いずれも極小のリクエストだが完全に無料ではない）。API Gatewayのスロットリングはステージ全体の合算のため（ADR-027）、ウォームアップの分だけ枠を消費する。`warmup()`は`/health`を1回叩くだけなので、複数の同時実行環境が必要な場面（同時アクセス）ではその全てを温められるわけではない。また実行環境の生存時間（AWS側の裁量）を超えて放置されれば再びコールドになるため、これは待ち時間を減らす確率的な緩和策であって保証ではない。
- **関連**: ADR-026（docling/pdf2htmlexのLambda化・SigV4署名）、ADR-027（ステージ単位スロットリング）、ADR-021（Supabase・RLS）。
