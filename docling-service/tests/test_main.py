...
# ADR-018: backend/app/services/docling_client.py（HTTP版）が期待する内部契約
# 「POST /convert に file を送ると {"html": ...} が返る」を先に定義する（Red状態）。
...
def test_convert_returns_html_on_success():
    class _FakeConverter:
        def convert_to_html(self, filename: str, content: bytes) -> str:
            return "<p>converted-html-marker</p>"
    ...
    try:
        ...
        assert response.json() == {"html": "<p>converted-html-marker</p>"}
    finally:
